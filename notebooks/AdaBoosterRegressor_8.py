...
def main(rootpath, loader):
    ...
    with open(os.path.join(rootpath, 'snapshots', 'AdaBoostRegressor.pkl'), 'wb') as file:
    ...
    loading.info("AdaBoost Regression Model: Return the Mean Squared Error and R-squared")
    ...
    plt.title('Residuals Plot for AdaBooster Regression')
    plt.savefig(os.path.join(rootpath, 'outputs', 'AdaBooster - Residuals Plot.jpg'), format='jpeg')
    ...
    plt.title('Actual vs. Predicted Base Fare using AdaBooster Regression')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(rootpath, 'outputs', 'AdaBooster - Actual vs. Predicted.jpg'), format='jpeg')

    loading.info("AdaBoost completed")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AdaBooster Regression')
